...
«Требование → приватный live canvas в боковой панели → два семантических батча с точным PNG для пользователя → детерминированные нативные/DSH-гейты качества → атомарная публикация» — полный цикл внутри DSH.
...
| `openpencil_pipeline_begin` | Запускает принадлежащий сессии приватный черновик с единственным root, немедленно открывает тот же live canvas в боковой панели и оставляет целевой `.op` неопубликованным. |
| `openpencil_pipeline_context` | Загружает одну ограниченную деталь guideline, style, theme или UI kit, только если её действительно нет в контракте begin; это не цикл обновления при запуске. |
| `openpencil_pipeline_batch` | Выполняет не более двух прямых QuickJS-скриптов генерации. Каждая успешная транзакция лишь пытается показать пользователю точный PNG; после возврата инструмента сразу следуйте `next`. |
| `openpencil_pipeline_inspect` | Предоставляет ручную диагностику только по явному запросу пользователя; обычная генерация не использует его как этап превью или просмотра изображения моделью. |
| `openpencil_pipeline_finish` | Выполняет детерминированные нативные и DSH-гейты, рендерит точный PNG после финализации и атомарно публикует через `createIfAbsent`. Ровно один U-only ремонт и один последний finish разрешены только при `needs_correction`, `canContinue: true`, полном непустом `repairTargets` и `omitted: 0`; любой другой неопубликованный результат терминален. |
...
Для обычной генерации используйте фиксированный короткий путь: `openpencil_pipeline_begin` → два прямых QuickJS-вызова `openpencil_pipeline_batch` → один `openpencil_pipeline_finish`. Begin создаёт единственный root и немедленно открывает приватный live canvas в боковой панели; запрошенный путь в рабочей области не существует до успешной публикации. После каждого успешного begin или батча немедленно выполняйте следующий обязательный вызов без повествования, планирования, сравнения, инспекции или посторонних инструментов. По умолчанию весь дизайн использует ровно одно изображение; каждый Hero/Product/Art/Media frame содержит ровно один основной визуал, никогда одновременно изображение и placeholder-иконку.

Оба успешных батча пытаются показать пользователю точное PNG-превью. После возврата инструмента сразу следуйте `next`. Если `next` сообщает `previewUnavailable`, скрипт уже закоммичен в live canvas: не запускайте батч повторно и не вызывайте `openpencil_pipeline_inspect` или `read_image`. `openpencil_pipeline_inspect` предназначен только для диагностики, явно запрошенной пользователем.

Finish выполняет нативные финализацию OpenPencil, lint, проверки контраста и макета, а также детерминированные гейты качества DSH; затем в том же здоровом вызове рендерит точный финальный PNG и атомарно публикует цель. Ремонт разрешён только если результат одновременно содержит `stage: "needs_correction"`, `canContinue: true`, полный непустой `repairTargets` с `omitted: 0`, а каждая цель — `operation: "U"`, точный непустой `nodeId` и непустой `patch`. Примените все цели вместе ровно в одном U-only батче, затем без пояснений вызовите finish ровно один последний раз. Любой другой неопубликованный результат, ошибка или `canContinue: false` терминальны: сообщите один раз и не повторяйте, не инспектируйте, не читайте image/context, не вызывайте abort и не создавайте заменяющий draft. При провале гейта или вызове `openpencil_pipeline_abort` цель не создаётся. Точный финальный PNG и live editor в published result уже авторитетны: верните результат и немедленно завершите работу. Боковая панель автоматически открывается только когда свободна, а **Редактировать холст** остаётся для явного переключения.
