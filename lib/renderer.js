...
async function readKey(path) {
    ...
    if (info.isSymbolicLink() || !info.isFile())
        throw new Error('render access key is not a regular file');
    const key = await readFile(path);
    if (key.length !== KEY_BYTES)
        throw new Error('render access key has an invalid length');
    return key;
}
/** Load or atomically create the per-DSH-home signing key. */
export async function prepareRenderAccessKey() {
...
function parsePayload(value) {
    ...
    if (value.v === 2) {
        if ((value.kind !== 'image' && value.kind !== 'document')
            ...
            return undefined;
        if (value.kind === 'image' && !/^render-[A-Za-z0-9-]+\.png$/.test(value.filename))
            return undefined;
        if (value.kind === 'document' && value.filename !== `${value.sha256}.op`)
            return undefined;
        return value;
    }
    if (value.v !== 1
...
function payloadMimeType(payload) {
...
async function assertNoSymlinkPath(root, path) {
    ...
    if (rootInfo.isSymbolicLink() || !rootInfo.isDirectory())
        throw new Error('render root is not a real directory');
    ...
    for (let index = 0; index < parts.length; index += 1) {
        ...
        if (part === undefined || part.length === 0 || part === '.' || part === '..')
            throw new Error('render path is malformed');
        current = join(current, part);
        const info = await lstat(current);
        if (info.isSymbolicLink())
            throw new Error('render path contains a symbolic link');
        const final = index === parts.length - 1;
        if (final ? !info.isFile() : !info.isDirectory())
            throw new Error('render path contains an unexpected entry type');
    }
...
async function openVerifiedRender(payload) {
    ...
    try {
        ...
        if (!info.isFile() || info.size !== payload.bytes)
            throw new Error('render no longer matches its delivered descriptor');
        ...
        if (payload.v === 2) {
            ...
            if (digest !== payload.sha256)
                throw new Error('artifact content hash changed');
        }
        await assertNoSymlinkPath(root, path);
        return { handle, info };
    }
...
export class RenderAccessController {
    ...
    constructor(key) {
        this.key = key;
        if (key.length !== KEY_BYTES)
            throw new TypeError(`render access key must be ${KEY_BYTES} bytes`);
    }
    ...
    signArtifact(artifact) {
        const payload = { v: 2, ...artifact };
        const parsed = parsePayload(payload);
        if (parsed === undefined || parsed.v !== 2)
...
export function expandUserHome(raw) {
    if (raw === '~')
        return homedir();
...
export function findOpenPencilBinary() {
    ...
    for (const dir of pathEnv.split(delimiter)) {
        if (dir.length > 0)
            candidates.push(join(dir, 'openpencil-desktop'));
    }
...
export function findJianBinary() {
...
export function parseRenderSize(stdout) {
...
export async function runOpenPencilRender(options) {
    ...
    const env = { ...process.env, OPENPENCIL_RENDER_MARGIN: '0' };
    ...
    try {
        const code = await new Promise((resolveCode, reject) => {
            child.on('error', (error) => {
                if (error.code === 'ENOENT')
                    reject(new RendererBinaryMissingError(`OpenPencil binary not found: ${options.binary}`, { cause: error }));
                else
                    reject(error);
            });
 ...
 */
export function projectRenderGrant(value, controller, viewer, editor) {
    if (!controller.routeAvailable || !isRecord(value))
        return value;
