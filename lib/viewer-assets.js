...
function parseManifest(value) {
    ...
    if (!isRecord(value.files))
        throw new Error('viewer asset manifest has no files table');
    const files = {};
...
async function verifyAssetDirectory(assetDir) {
    const directory = await realpath(assetDir);
    const rootInfo = await lstat(directory);
    if (!rootInfo.isDirectory())
        throw new Error('viewer asset root is not a directory');
    ...
    for (const name of VIEWER_ASSET_FILES) {
        ...
        if (!isContained(directory, path))
            throw new Error(`viewer asset escaped its root: ${name}`);
...
function candidateDirectories(explicit) {
    const configured = explicit?.trim() || process.env[VIEWER_ASSET_DIR_ENV]?.trim();
    if (configured)
        return { explicit: true, paths: [resolve(configured)] };
...
function baseSecurityHeaders(res) {
    res.setHeader('X-Content-Type-Options', 'nosniff');
    res.setHeader('Referrer-Policy', 'no-referrer');
    res.setHeader('Cross-Origin-Resource-Policy', 'same-origin');
    res.setHeader('Cache-Control', 'private, max-age=31536000, immutable');
}
function sameFile(info, asset) {
    if (info.dev === 0 || info.ino === 0 || asset.dev === 0 || asset.ino === 0)
        return true;
    return info.dev === asset.dev && info.ino === asset.ino;
}
async function openVerifiedAsset(asset) {
...
export class ViewerAssetController {
    ...
    assets;
    routeCount = 0;
    constructor(assetDirectory, unavailableReason, revision, assets) {
    ...
    static ready(directory, revision, assets) {
    ...
    get viewerGrant() {
        if (!this.routeAvailable || this.revision === undefined)
            return undefined;
    ...
    attachRoute() {
        if (!this.available)
            throw new Error(this.unavailableReason ?? 'OpenPencil viewer assets are unavailable');
        this.routeCount += 1;
        let active = true;
        return () => {
            if (!active)
...
export async function prepareViewerAssets(options = {}) {
    ...
    for (const path of candidates.paths) {
        try {
            ...
            if (!candidates.explicit && (code === 'ENOENT' || code === 'ENOTDIR'))
                continue;
