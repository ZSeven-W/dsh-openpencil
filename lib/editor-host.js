...
function deriveEditorKey(masterKey) {
    if (masterKey.length !== 32)
        throw new TypeError('editor master key must be 32 bytes');
    return Buffer.from(hkdfSync('sha256', masterKey, Buffer.from('dsh-openpencil/editor-key/v1'), Buffer.from('aes-256-gcm'), 32));
}
...
async function readSourceDocument(path) {
    const before = await lstat(path);
    if (before.isSymbolicLink() || !before.isFile())
        throw new HttpError(409, 'OpenPencil source is no longer a regular file');
...
export function findEditorHostBinary() {
    ...
    for (const dir of (process.env.PATH ?? '').split(delimiter)) {
        if (dir.length > 0)
            candidates.push(join(dir, 'op-host-web-server'));
    }
...
function sourceRootForBinary(binary) {
    ...
    return candidates.find(root => (isRegularFile(join(root, 'crates', 'op-host-web', 'pkg', 'op_host_web.js'))
        && isRegularFile(join(root, 'crates', 'op-host-web', 'assets', 'canvaskit', 'canvaskit.wasm'))));
}
function requestOrigin(req) {
    const raw = req.headers.origin;
    if (typeof raw !== 'string')
        throw new HttpError(403, 'editor launch requires a browser Origin header');
    let url;
    try {
        url = new URL(raw);
    }
    ...
    if (!loopback)
        throw new HttpError(403, 'editable OpenPencil sessions are restricted to loopback DSH');
    return url.origin;
}
...
async function readRequestBody(req) {
    ...
    for await (const chunk of req) {
        ...
        size += bytes.length;
        if (size > MAX_DOCUMENT_BYTES + 1024 * 1024)
            throw new HttpError(413, 'editor request is too large');
        chunks.push(bytes);
    }
...
async function waitForHandshake(child, diagnostics) {
    return new Promise((resolveHandshake, rejectHandshake) => {
        let settled = false;
        let stdout = '';
        const finish = (error, handshake) => {
            if (settled)
                return;
            settled = true;
            clearTimeout(timer);
...
async function waitForEditorReady(baseUrl) {
    ...
    while (Date.now() < deadline) {
        try {
            ...
            if (root.status === 200 && glue.status === 200)
                return;
...
async function atomicWriteDocument(path, text) {
    ...
    if (bytes.length === 0 || bytes.length > MAX_DOCUMENT_BYTES)
        throw new HttpError(413, 'OpenPencil document size is invalid');
    let parsed;
    try {
...
export class EditorHostController {
    ...
    get available() { return this.binary !== undefined; }
    get routeAvailable() { return this.#routeRefs > 0; }
    attachRoute() {
        this.#routeRefs += 1;
        let attached = true;
        return () => {
    ...
    grantFor(sourcePath, sourceSha256) {
        if (!this.available || !this.routeAvailable || sourcePath === undefined || sourceSha256 === undefined)
            return undefined;
        if (!isAbsolute(sourcePath) || !sourcePath.toLowerCase().endsWith('.op') || !isSha256(sourceSha256))
            return undefined;
        const now = Date.now();
    ...
    async #save(id, req, res) {
        ...
        try {
            ...
            if (typeof value !== 'object' || value === null || Array.isArray(value))
                throw new HttpError(400, 'editor save request is invalid');
            const record = value;
            if (record.sessionId !== id || typeof record.docJson !== 'string')
                throw new HttpError(400, 'editor save request is incomplete');
